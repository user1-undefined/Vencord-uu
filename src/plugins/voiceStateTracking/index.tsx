...
function MakeContextCallback(name: "Guild" | "Channel" | "User"): NavContextMenuPatchCallback {
    return (children, props) => {
        ...
        const container = findGroupChildrenByChildId(`devmode-copy-id-${_id}`, children);

        const add_action = () => {
            if (!_id) return;
            const ids_str = settings.store[ids_key];
        ...
        const remove_action = () => {
            ...
            const ids = parseIds(ids_str);
            if (!ids.includes(_id)) return;
            settings.store[ids_key] = ids.filter(i => i !== _id).join(",");
        };
...
const ClearChatBarButton: ChatBarButtonFactory = ({ channel: { id: channelId } }) => {
...
function clearMessages(channelId: string, msgs: Message[]) {
    ...
    settings.store.created = (settings.store.created as number) - msgs.length;
}

const parseIds = (ids: string | undefined, sep: string = ",") => (ids?.split(sep) || []).flatMap(i => (i = i.trim()) !== "" ? [i] : []);

export const createdMessages = new Map<string, Message[]>();

const plugin = definePlugin({
    name: "VoiceStateTracking",
    description: "Logging users (join, leave, move) between voice channels in chat",
    tags: ["Chat", "Utility", "Accessibility", "Notifications", "Activity"],
    authors: [Devs.uu],
    ...
    toolboxActions: {
        "VoiceStateTracking Settings": () => openPluginModal(plugin),
    },
